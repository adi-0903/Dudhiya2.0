...
import { getProRataPurchaseSummaryReportCollections, generateProRataPurchaseSummaryReport } from '../../services/api';
...
const GenerateProRataPurchaseSummaryReportScreen = ({ navigation }) => {
  ...
  const fetchCollections = async (pageNumber = 1, isLoadMore = false) => {
    try {
      ...
      const response = await getProRataPurchaseSummaryReportCollections(apiParams);
      
      console.log('Purchase summary response:', response);
      
      // Handle different response structures - the API returns summary_data
      let dataArray = [];
      if (Array.isArray(response)) {
        dataArray = response;
      } else if (response && response.summary_data) {
        dataArray = response.summary_data;
      ...
      if (dataArray && dataArray.length > 0) {
        const formattedCollections = dataArray
          .map(collection => ({
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      const fileUri = await generateProRataPurchaseSummaryReport(startDate, endDate);
...
export default GenerateProRataPurchaseSummaryReportScreen;
