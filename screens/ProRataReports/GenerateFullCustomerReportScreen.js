...
import { getCollections, generateProRataCustomerBills } from '../../services/api';
...
const GenerateFullProRataCustomerReportScreen = ({ navigation }) => {
  ...
  // Prevent FlatList from auto-calling onEndReached when content doesn't fill the screen
  ...
  useFocusEffect(
    React.useCallback(() => {
      console.log('generateFullCustomerReportScreen focused - refreshing data');
      fetchCollections();
    }, [])
  );
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      const fileUri = await generateProRataCustomerBills(startDate, endDate);
  ...
  const fetchFilteredCollections = async (fromDate, toDate, pageNumber = 1, isLoadMore = false) => {
    try {
      ...
      if (response && response.results) {
        ...
        // Additional safety check to ensure dates are within range and filter to show only pro rata collections
        const formattedCollections = response.results
          .filter(collection => {
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={styles.headerTitleContainer}>
          <Text style={styles.headerTitle}>{t('all supplier bill')}</Text>
        ...
        <View style={styles.scrollContainer}>
          {isLoading ? (
            ...
            <FlatList
              ...
              // Only allow loading more after user scrolls (prevents initial auto-loop)
...
export default GenerateFullProRataCustomerReportScreen;
