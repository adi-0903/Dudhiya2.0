...
import { getProRataPurchaseReportCollections, generateProRataPurchaseReport } from '../../services/api';
...
const GenerateProRataPurchaseReportScreen = ({ navigation }) => {
  ...
  const fetchCollections = async (pageNumber = 1, isLoadMore = false) => {
    try {
      ...
      const response = await getProRataPurchaseReportCollections({ page: pageNumber, page_size: 50 });
      ...
      .map(collection => {
          // Ensure each collection has a valid id
          ...
          return {
            ...
            weight: collection.total_weight,
            fat_percentage: collection.total_fat_percentage,
            snf_percentage: collection.total_snf_percentage,
            fat_kg: collection.total_fat_kg,
            snf_kg: collection.total_snf_kg,
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      const fileUri = await generateProRataPurchaseReport(startDate, endDate);
  ...
  const renderCollection = ({ item }) => (
    <TouchableOpacity style={styles.collectionRow}>
      ...
      <Text style={styles.separator}>|</Text>
      <Text style={styles.cell}>{parseFloat(item.fat_kg).toFixed(1)}</Text>
      <Text style={styles.separator}>|</Text>
      <Text style={styles.cell}>{parseFloat(item.snf_percentage).toFixed(1)}</Text>
      <Text style={styles.separator}>|</Text>
      <Text style={styles.cell}>{parseFloat(item.snf_kg).toFixed(1)}</Text>
      <Text style={styles.separator}>|</Text>
      <Text style={[styles.cell, styles.amountCell]}>₹{Math.round(item.amount)}</Text>
    </TouchableOpacity>
  );
  ...
  const HeaderRow = () => (
    <View style={styles.headerRow}>
      ...
      <Text style={[styles.separator, styles.headerSeparator]}>|</Text>
      <Text style={[styles.headerCell, styles.kgCell]}>Fat{'\n'}KG</Text>
      <Text style={[styles.separator, styles.headerSeparator]}>|</Text>
      <Text style={styles.headerCell}>SNF%</Text>
      <Text style={[styles.separator, styles.headerSeparator]}>|</Text>
      <Text style={[styles.headerCell, styles.kgCell]}>SNF{'\n'}KG</Text>
      <Text style={[styles.separator, styles.headerSeparator]}>|</Text>
  ...
  const fetchFilteredCollections = async (fromDate, toDate, pageNumber = 1, isLoadMore = false) => {
    try {
      ...
      console.log('Fetching collections with dates:', { fromDateStr, toDateStr, pageNumber });
      const response = await getProRataPurchaseReportCollections({ 
      ...
      if (response && response.results) {
        const formattedCollections = response.results
          .filter(collection => {
            ...
            // Ensure the collection date is within the range
        ...
        console.log('Filtered collections:', formattedCollections.length);
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={styles.headerTitleContainer}>
          <Text style={styles.headerTitle}>{t('purchase report')}</Text>
          <Text style={styles.headerSubtitle}>{t('pro-rata')}</Text>
        </View>
        <TouchableOpacity 
...
const styles = StyleSheet.create({
  ...
  separator: {
    width: '1%',
    fontSize: 12, // Matched separator size
    color: '#ccc',
    textAlign: 'center',
  },
  headerSeparator: {
...
export default GenerateProRataPurchaseReportScreen; 
