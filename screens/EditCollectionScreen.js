...
import { getCollection, getAllCustomers, getCustomers, updateCollection, deleteCollection, generateCustomerReport, getDairyInfo, updateDairyInfo } from '../services/api';
...
const EditCollectionScreen = ({ route, navigation }) => {
  ...
  // Update SNF from CLR when CLR and fat percentage change
  useEffect(() => {
    if (selectedRadios.clr && formData.clr && formData.fat_percentage) {
      ...
      if (calculatedSnf) {
        setFormData(prev => ({ ...prev, snf_percentage: calculatedSnf }));
      }
    }
  }, [formData.clr, formData.fat_percentage, selectedRadios.clr, clrConversionFactor]);
  ...
  const handleClrRadioPress = () => {
    ...
    // Clear SNF input when switching to CLR and set CLR to empty
    setFormData(prev => ({ 
      ...prev, 
      snf_percentage: '',
      clr: '' // Start with empty CLR field instead of showing 0.00
    }));
    setIsDirty(true);
  };
  ...
  const handleSnfRadioPress = () => {
    ...
    // Clear CLR input when switching to SNF and set SNF to empty
    setFormData(prev => ({ 
      ...prev, 
      clr: '',
      snf_percentage: '' // Also clear the SNF field
    }));
    setIsDirty(true);
  };
    ...
    if (validateInputs()) {
      try {
        ...
        if (selectedRadios.clr) {
          ...
          snfPercentage = calculateSnfFromClr(clr, fatPercentage);
        } else {
        ...
        let clrValue;
        if (selectedRadios.clr) {
          clrValue = formData.clr; // Use the exact value entered by the user
        } else {
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.changeRatesButtonContainer}>
          <TouchableOpacity 
            style={styles.changeRatesButton}
            onPress={() => {
          ...
          >
            <View style={styles.changeRatesButtonContent}>
              <Text style={styles.changeRatesButtonLabel}>{t('rate settings')}</Text>
            </View>
        ...
        <View style={styles.row}>
          <View style={styles.inputGroup}>
            ...
            <TextInput
              value={formData.clr}
              onChangeText={(text) => {
                // Allow direct input of the CLR value, preserving exactly what the user types
                // Only basic validation: remove non-numeric characters except decimal point
                const sanitizedText = text.replace(/[^0-9.]/g, '');
                ...
                // Don't limit decimal places or format the value in any way
                // Let the user enter the exact value they want
                handleInputChange('clr', sanitizedText);
              }}
              keyboardType="decimal-pad"
              style={[
                styles.textInput, 
        ...
        {/* Display radio error if neither SNF nor CLR is selected */}
        {errors.radio && (
        ...
        >
          <TouchableWithoutFeedback>
            <View style={styles.changeRatesModalContent}>
              ...
              <ScrollView style={styles.changeRatesScrollView}>
...
const styles = StyleSheet.create({
  ...
  changeRatesButtonLabel: {
    fontSize: 14,
    fontWeight: '600',
    color: '#fff',
  },
  // Rate Settings Modal Styles
