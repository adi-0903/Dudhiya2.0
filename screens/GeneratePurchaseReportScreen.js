...
import { getPurchaseReportCollections, generatePurchaseReport } from '../services/api';
...
const GeneratePurchaseReportScreen = ({ navigation }) => {
  ...
  const fetchCollections = async (pageNumber = 1, isLoadMore = false) => {
    try {
      ...
      const response = await getPurchaseReportCollections({ page: pageNumber, page_size: 50 });
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      const fileUri = await generatePurchaseReport(startDate, endDate);
  ...
  const fetchFilteredCollections = async (fromDate, toDate, pageNumber = 1, isLoadMore = false) => {
    try {
      ...
      const response = await getPurchaseReportCollections({ 
...
export default GeneratePurchaseReportScreen; 
